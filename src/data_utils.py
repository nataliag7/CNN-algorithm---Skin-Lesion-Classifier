...
def scan_isic_dir(dirpath):
    ##Return dicts {stem: path} for photos and masks in an ISIC-style folder. Masks end with '_Segmentation' in the filename.
    images, masks = {}, {}
...
def to_stem(s):
    ##Convert an image filename (or path) to its base stem. Also strips '_Segmentation' from mask filenames.
...
def crop_to_mask_with_margin(img, mask, has_mask, margin=0.10):
    ##Crop image to the bounding box of the mask plus a margin (fraction of bbox size). If there is no mask or it's empty, just return the original image.
    def _crop():
        #mask: (H, W, 1) -> (H, W)
        m = tf.squeeze(mask, -1)
